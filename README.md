...
纳博特（inexbot）机器人控制系统的技术顾问skill，涵盖产品选型、配置调试、二次开发、工艺应用全流程。文档索引基于 doc.inexbot.com（552篇文档，含349篇独立伺服报错页面，含25.01版本41篇新文档，含13篇行业方案）。2026-07-23 站点行业方案新增 11 篇文档（轮式人形机器人运动控制系统、免编程示教系统喷涂、打磨、晶圆、激光寻位-跟踪、激光焊接、点胶、物流系统自动供包、码垛、金属焊接、医疗机器人）。每次回答问题后自动追加 Q&A 到下方缓存区。
...
- **类型**：SKILL.md-only drift sync（第 5 种变更模式，详见 `references/skill-drift-sync.md`）
- **站点变化**：0 篇新增 / 0 篇移除 / hash map 完全未变
- **本地变更**：SKILL.md 新增 2026-07-24 cron 历史记录 + Layer 4 drift 检测流程文档
- **上传文件**：6 个（README + 5 格式），跳过 `hash-map-snapshot.json`
- **三方比对**：site (36858B) == local (36858B) == GitHub SHA `f8592acb` (36858B)，全部字节相同
