...
- **类型**：SKILL.md-only drift sync（第 5 种变更模式，cron 第 5 次捕获 drift）
- **站点变化**：0 篇新增 / 0 篇移除 / hash map 完全未变（自 2026-07-23 起 552 篇文档稳定 4 天）
- **本地变更**：SKILL.md 累积 1224B drift（frontmatter description 末尾追加 + 顶部新增更详细历史条目 + VitePress 重建陷阱章节追加 2026-07-27 行）
- **上传文件**：6 个（README + 5 格式），跳过 `hash-map-snapshot.json`
- **三方比对**：site (36858B) == local (36858B) == GitHub SHA `f8592acb` (36858B)，全部字节相同
- **SKILL.md 字节**：本地 52426B vs GitHub-hermes 49770B = 2656B drift（已修复）
