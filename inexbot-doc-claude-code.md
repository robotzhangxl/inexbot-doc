...
> 🕐 上次自动同步: 2026-07-31 — **BLOCKED_PAT_REVOKED**（0 上传）。GitHub PAT `ghp_QK...DS4v` 已被撤销，api.github.com 全部端点 401 Bad credentials。站点 9 天无变化（552 篇 / 36858B / md5=`b6cdef7f`）。本地 SKILL.md 57185B md5=`dd5b5d393e1787c82f8c8fa7396d2466` vs GitHub hermes 46806B md5=`4e79dfdf6d5579b7` = **+10379B drift**（待 PAT 轮换后用 SKILL.md-only drift sync 闭合）。**重大发现 1**：2026-07-30 cron 报告的 6 文件上传 (commit SHA `cfe1ced1`/`dc4be437`/`791d0a0f`/`aa75f4c2`/`de320ddb`/`ed5266f3` + 闭环验证 hermes md5=`375a9db002d11c32` 55269B) 经实测为**假阳性**——2026-07-31 实际 GitHub hermes 仍为 `0e22b6696a785c5f` 46806B，与 2026-07-30 "before" 值一致；6 个新 commit 实际从未落在 main 上。**重大发现 2**：`check_today.py` 用 `subprocess.run(capture_output=True)` 静默吞掉 curl exit 7 / 401，凭 stale `/tmp/gh_hash.json` (mtime 2026-07-27) 输出"Layer 1/2/3/4 OK"假成功。代理 `127.0.0.1:7890` 已死，必须 `--noproxy '*'`；`check_today.py` 未遵循此 SOP。详见 `references/cron-run-2026-07-31-pat-revoked.md`。下次 cron 前置：① 轮换 PAT + 更新所有 `_B64` 脚本 + `/tmp/inexbot-doc/_token.txt`；② 修复 `check_today.py` 去掉 `capture_output`；③ 重新跑 6 文件 drift-sync 闭合 +10379B drift。
