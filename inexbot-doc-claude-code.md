---
name: inexbot-doc
description: 纳博特（inexbot）机器人控制系统的技术顾问skill，涵盖产品选型、配置调试、二次开发、工艺应用全流程。文档索引基于 doc.inexbot.com（552篇文档，含349篇独立伺服报错页面，含25.01版本41篇新文档，含13篇行业方案）。2026-07-23 站点行业方案新增 11 篇文档（轮式人形机器人运动控制系统、免编程示教系统喷涂、打磨、晶圆、激光寻位-跟踪、激光焊接、点胶、物流系统自动供包、码垛、金属焊接、医疗机器人）。2026-08-07 站点真实更新：+1 新增 C2202 嵌入式控制主板，−1 移除 技术资料_伺服报错代码 聚合页（迁移至 常见问题_伺服报错_index）。每次回答问题后自动追加 Q&A 到下方缓存区。2026-07-27 cron：第 5 次捕获 SKILL.md-only drift (1224B)，三方 hashmap 仍全等 36858B，6 文件上传。2026-08-09 cron：第 8 次捕获 SKILL.md-only drift，站点三方全等（hashmap md5 `c1960907`，Last-Modified 07 Aug），本地 SKILL.md +2143B drift，6 文件上传闭环 Equal：True。2026-08-10 cron：第 9 次捕获 SKILL.md-only drift，站点三方全等（hashmap md5 `c1960907`，Last-Modified 07 Aug，无文档更新），本地 SKILL.md +581B drift，6 文件上传闭环 Equal：True。2026-08-11 cron：真实内容更新，22.07 冲压工艺手册重组为 冲压工艺2.0手册 + 冲压工艺3.0手册，−2/+2，7 文件全传。2026-08-12 cron：SKILL.md-only drift（第10次捕获 +1886B），站点三方全等（hashmap md5 `e2bbec2b`，无文档更新），本地 SKILL.md drift，6 文件上传闭环 Equal：True。
---


# 纳博特（inexbot）机器人控制系统 — 技术顾问

> 🕐 上次自动同步: 2026-08-12 — SKILL.md-only drift sync（第 10 次捕获 drift，mode 5）：站点 hashmap 三方全等（site 36834B/md5 `e2bbec2b` == baseline == GitHub snapshot，Last-Modified 2026-08-11，**无文档更新**）。本地 SKILL.md 50289 chars（md5 `f07736e3`）≠ GitHub hermes 48403 chars（md5 `94f894e9`）= **+1886B drift**（08-11 真实更新上传后，SKILL.md 后续累积：Q&A 缓存区、references 指针、PROFINET 集成条目、08-11 实录 SOP）。复用 skill 内置 `scripts/generate_formats.py` 全链路：重生成 6 文件 → 上传 6 文件（跳过 hash-map-snapshot.json，三方全等）→ 闭环验证 GitHub hermes md5=`f07736e3` == 本地 SKILL.md（**Equal: True**）。6 个 commit（`e0b1d17e`/`66f19195`/`bdc1140e`/`9b7f30d4`/`5c6f65fb`/`5fe2022f`）已落 main。README 本轮**已更新**（description 变更 → README 嵌入 frontmatter description，非 SKIP）。
